...
from sfdt_ibrahim import __version__
...
ASSETS_URL = "https://github.com/sfdt_ibrahim/assets/releases/download/v0.0.0"  # assets GitHub URL
DEFAULT_CFG_PATH = ROOT / "cfg/default.yaml"
DEFAULT_SOL_CFG_PATH = ROOT / "cfg/solutions/default.yaml"  # SFDT_Ibrahim solutions yaml path
NUM_THREADS = min(8, max(1, os.cpu_count() - 1))  # number of YOLO multiprocessing threads
AUTOINSTALL = str(os.getenv("YOLO_AUTOINSTALL", True)).lower() == "true"  # global auto-install mode
VERBOSE = str(os.getenv("YOLO_VERBOSE", True)).lower() == "true"  # global verbose mode
TQDM_BAR_FORMAT = "{l_bar}{bar:10}{r_bar}" if VERBOSE else None  # tqdm bar format
LOGGING_NAME = "sfdt_ibrahim"
...
HELP_MSG = """
    Examples for running SFDT_Ibrahim:

    1. Install the sfdt_ibrahim package:
    ...
    3. Use the command line interface (CLI):
        ...
        SFDT_Ibrahim 'yolo' CLI commands use the following syntax:
            ...
            Where   TASK (optional) is one of [detect, segment, classify, pose, obb]
                    ...
                    ARGS (optional) are any number of custom "arg=value" pairs like "imgsz=320" that override defaults.
                        See all ARGS at https://docs.sfdt_ibrahim.com/usage/cfg or with "yolo cfg"
    ...
    Docs: https://docs.sfdt_ibrahim.com
    Community: https://community.sfdt_ibrahim.com
    GitHub: https://github.com/sfdt_ibrahim/sfdt_ibrahim
    """
...
class TQDM(rich.tqdm if TQDM_RICH else tqdm.tqdm):
    ...
    Examples:
        >>> from sfdt_ibrahim.utils import TQDM
    ...
    def __init__(self, *args, **kwargs):
        ...
        This class extends the original tqdm class to provide customized behavior for SFDT_Ibrahim projects.
...
class IterableSimpleNamespace(SimpleNamespace):
    ...
    def __getattr__(self, attr):
        ...
        raise AttributeError(
            f"""
            '{name}' object has no attribute '{attr}'. This may be caused by a modified or out of date sfdt_ibrahim
            'default.yaml' file.\nPlease update your code with 'pip install -U sfdt_ibrahim' and if necessary replace
            {DEFAULT_CFG_PATH} with the latest version from
            https://github.com/sfdt_ibrahim/sfdt_ibrahim/blob/main/sfdt_ibrahim/cfg/default.yaml
            """
        )
...
def set_logging(name="LOGGING_NAME", verbose=True):
    ...
    This function configures logging for the SFDT_Ibrahim library, setting the appropriate logging level and
    ...
    Examples:
        >>> set_logging(name="sfdt_ibrahim", verbose=True)
        >>> logger = logging.getLogger("sfdt_ibrahim")
...
class ThreadingLocked:
    ...
    Example:
        ```python
        from sfdt_ibrahim.utils import ThreadingLocked
...
DEFAULT_SOL_DICT = yaml_load(DEFAULT_SOL_CFG_PATH)  # SFDT_Ibrahim solutions configuration
...
def get_user_config_dir(sub_dir="SFDT_Ibrahim"):
    """
...
USER_CONFIG_DIR = Path(os.getenv("YOLO_CONFIG_DIR") or get_user_config_dir())  # SFDT_Ibrahim settings dir
...
class TryExcept(contextlib.ContextDecorator):
    """
    SFDT_Ibrahim TryExcept class. Use as @TryExcept() decorator or 'with TryExcept():' context manager.
...
class SettingsManager(JSONDict):
    """
    SettingsManager class for managing and persisting SFDT_Ibrahim settings.
    ...
    def __init__(self, file=SETTINGS_FILE, version="0.0.6"):
        ...
        from sfdt_ibrahim.utils.torch_utils import torch_distributed_zero_first
        ...
        self.help_msg = (
            f"\nView SFDT_Ibrahim Settings with 'yolo settings' or at '{self.file}'"
            "\nUpdate Settings with 'yolo settings key=value', i.e. 'yolo settings runs_dir=path/to/dir'. "
            "For help see https://docs.sfdt_ibrahim.com/quickstart/#sfdt_ibrahim-settings."
        )
        ...
        with torch_distributed_zero_first(RANK):
            ...
            if not self.file.exists() or not self:  # Check if file doesn't exist or is empty
                LOGGER.info(f"Creating new SFDT_Ibrahim Settings v{version} file ✅ {self.help_msg}")
                self.reset()
    ...
    def _validate_settings(self):
        ...
        if not (correct_keys and correct_types and correct_version):
            LOGGER.warning(
                "WARNING ⚠️ SFDT_Ibrahim settings reset to default values. This may be due to a possible problem "
                f"with your settings or a recent sfdt_ibrahim package update. {self.help_msg}"
            )
            self.reset()

        if self.get("datasets_dir") == self.get("runs_dir"):
            LOGGER.warning(
                f"WARNING ⚠️ SFDT_Ibrahim setting 'datasets_dir: {self.get('datasets_dir')}' "
    ...
    def update(self, *args, **kwargs):
        ...
        for k, v in kwargs.items():
            if k not in self.defaults:
                raise KeyError(f"No SFDT_Ibrahim setting '{k}'. {self.help_msg}")
            t = type(self.defaults[k])
            if not isinstance(v, t):
                raise TypeError(
                    f"SFDT_Ibrahim setting '{k}' must be '{t.__name__}' type, not '{type(v).__name__}'. {self.help_msg}"
                )
        super().update(*args, **kwargs)
...
def vscode_msg(ext="sfdt_ibrahim.sfdt_ibrahim-snippets") -> str:
    """Display a message to install SFDT_Ibrahim-Snippets for VS Code if not already installed."""
    path = (USER_CONFIG_DIR.parents[2] if WINDOWS else USER_CONFIG_DIR.parents[1]) / ".vscode/extensions"
    obs_file = path / ".obsolete"  # file tracks uninstalled extensions, while source directory remains
    installed = any(path.glob(f"{ext}*")) and ext not in (obs_file.read_text("utf-8") if obs_file.exists() else "")
    url = "https://docs.sfdt_ibrahim.com/integrations/vscode"
    return "" if installed else f"{colorstr('VS Code:')} view SFDT_Ibrahim VS Code Extension ⚡ at {url}"


# Run below code on utils init ------------------------------------------------------------------------------------

# Check first-install steps
PREFIX = colorstr("SFDT_Ibrahim: ")
...
from sfdt_ibrahim.utils.patches import imread, imshow, imwrite, torch_load, torch_save
