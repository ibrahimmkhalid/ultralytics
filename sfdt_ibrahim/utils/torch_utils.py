...
if WINDOWS and check_version(torch.__version__, "==2.4.0"):  # reject version 2.4.0 on Windows
    LOGGER.warning(
        ...
        "https://github.com/sfdt_ibrahim/sfdt_ibrahim/issues/15049"
    )
...
def get_cpu_info():
    ...
    from sfdt_ibrahim.utils import PERSISTENT_CACHE  # avoid circular import error
...
def select_device(device="", batch=0, newline=False, verbose=True):
    ...
    s = f"SFDT_Ibrahim {__version__} 🚀 Python-{PYTHON_VERSION} torch-{torch.__version__} "
...
def init_seeds(seed=0, deterministic=False):
    ...
    # torch.backends.cudnn.benchmark = True  # AutoBatch problem https://github.com/sfdt_ibrahim/yolov5/issues/9287
    if deterministic:
        if TORCH_2_0:
...
def strip_optimizer(f: Union[str, Path] = "best.pt", s: str = "", updates: dict = None) -> dict:
    ...
    Example:
        ```python
        from pathlib import Path
        from sfdt_ibrahim.utils.torch_utils import strip_optimizer
    ...
    Note:
        Use `sfdt_ibrahim.nn.torch_safe_load` for missing modules with `x = torch_safe_load(f)[0]`
    """
    try:
        ...
        LOGGER.warning(f"WARNING ⚠️ Skipping {f}, not a valid SFDT_Ibrahim model: {e}")
        return {}
...
def profile(input, ops, n=10, device=None, max_num_obj=0):
    """
    SFDT_Ibrahim speed, memory and FLOPs profiler.

    Example:
        ```python
        from sfdt_ibrahim.utils.torch_utils import profile
