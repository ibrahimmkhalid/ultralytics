...
from sfdt_ibrahim.data.utils import FORMATS_HELP_MSG, IMG_FORMATS, VID_FORMATS
from sfdt_ibrahim.utils import IS_COLAB, IS_KAGGLE, LOGGER, ops
from sfdt_ibrahim.utils.checks import check_requirements
from sfdt_ibrahim.utils.patches import imread
...
class LoadScreenshots:
    """
    SFDT_Ibrahim screenshot dataloader for capturing and processing screen images.
...
class LoadImagesAndVideos:
    ...
    def __init__(self, path, batch=1, vid_stride=1):
        ...
        for p in sorted(path) if isinstance(path, (list, tuple)) else [path]:
            a = str(Path(p).absolute())  # do not use .resolve() https://github.com/sfdt_ibrahim/sfdt_ibrahim/issues/2912
            if "*" in a:
...
def autocast_list(source):
    """Merges a list of sources into a list of numpy arrays or PIL images for SFDT_Ibrahim prediction."""
    files = []
    for im in source:
        if isinstance(im, (str, Path)):  # filename or uri
            ...
            raise TypeError(
                f"type {type(im).__name__} is not a supported SFDT_Ibrahim prediction source type. \n"
                f"See https://docs.sfdt_ibrahim.com/modes/predict for supported source types."
            )
