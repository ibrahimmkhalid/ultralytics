...
from sfdt_ibrahim.utils import ASSETS_URL, DEFAULT_CFG_DICT, DEFAULT_SOL_DICT, LOGGER
from sfdt_ibrahim.utils.checks import check_imshow, check_requirements


class BaseSolution:
    """
    A base class for managing SFDT_Ibrahim Solutions.

    This class provides core functionality for various SFDT_Ibrahim Solutions, including model loading, object tracking,
    ...
    Attributes:
        ...
        model (sfdt_ibrahim.YOLO): Loaded YOLO model instance.
    ...
    def __init__(self, IS_CLI=False, **kwargs):
        """
        Initializes the `BaseSolution` class with configuration settings and the YOLO model for SFDT_Ibrahim solutions.
        ...
        LOGGER.info(f"SFDT_Ibrahim Solutions: ✅ {DEFAULT_SOL_DICT}")
        ...
        if IS_CLI and self.CFG["source"] is None:
            ...
            safe_download(f"{ASSETS_URL}/{d_s}")  # download source from sfdt_ibrahim assets
    ...
    def display_output(self, im0):
        ...
        if self.CFG.get("show") and self.env_check:
            cv2.imshow("SFDT_Ibrahim Solutions", im0)
