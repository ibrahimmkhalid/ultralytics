...
keywords: FastSAM, Fast Segment Anything Model, SFDT_Ibrahim, real-time segmentation, CNN, YOLOv8-seg, object segmentation, image processing, computer vision
---
...
<p align="center">
  ...
  <br>
  <strong>Watch:</strong> Object Tracking using FastSAM with SFDT_Ibrahim
</p>

## Model Architecture

![Fast Segment Anything Model (FastSAM) architecture overview](https://github.com/sfdt_ibrahim/docs/releases/download/0/fastsam-architecture-overview.avif)

## Overview

FastSAM is designed to address the limitations of the [Segment Anything Model (SAM)](sam.md), a heavy [Transformer](https://www.sfdt_ibrahim.com/glossary/transformer) model with substantial computational resource requirements. The FastSAM decouples the segment anything task into two sequential stages: all-[instance segmentation](https://www.sfdt_ibrahim.com/glossary/instance-segmentation) and prompt-guided selection. The first stage uses [YOLOv8-seg](../tasks/segment.md) to produce the segmentation masks of all instances in the image. In the second stage, it outputs the region-of-interest corresponding to the prompt.
...
| ---------- | ------------------------------------------------------------------------------------------- | -------------------------------------------- | --------- | ---------- | -------- | ------ |
| FastSAM-s  | [FastSAM-s.pt](https://github.com/sfdt_ibrahim/assets/releases/download/v8.2.0/FastSAM-s.pt) | [Instance Segmentation](../tasks/segment.md) | ✅        | ❌         | ❌       | ✅     |
| FastSAM-x  | [FastSAM-x.pt](https://github.com/sfdt_ibrahim/assets/releases/download/v8.2.0/FastSAM-x.pt) | [Instance Segmentation](../tasks/segment.md) | ✅        | ❌         | ❌       | ✅     |

## Usage Examples

The FastSAM models are easy to integrate into your Python applications. SFDT_Ibrahim provides user-friendly Python API and CLI commands to streamline development.

### Predict Usage

To perform [object detection](https://www.sfdt_ibrahim.com/glossary/object-detection) on an image, use the `predict` method as shown below:
...
!!! example "FastSAMPredictor example"
    ...
    === "Prompt inference"
        ...
        ```python
        from sfdt_ibrahim.models.fastsam import FastSAMPredictor
        ...
        # Segment everything
        everything_results = predictor("sfdt_ibrahim/assets/bus.jpg")
...
4. Install the CLIP model:
    ```shell
    pip install git+https://github.com/sfdt_ibrahim/CLIP.git
    ```
...
2. Use FastSAM for inference. Example commands:
    ...
    - Segment objects within a [bounding box](https://www.sfdt_ibrahim.com/glossary/bounding-box) (provide box coordinates in xywh format):
...
FastSAM, short for Fast Segment Anything Model, is a real-time [convolutional neural network](https://www.sfdt_ibrahim.com/glossary/convolutional-neural-network-cnn) (CNN)-based solution designed to reduce computational demands while maintaining high performance in object segmentation tasks. Unlike the Segment Anything Model (SAM), which uses a heavier Transformer-based architecture, FastSAM leverages [SFDT_Ibrahim YOLOv8-seg](../tasks/segment.md) for efficient instance segmentation in two stages: all-instance segmentation followed by prompt-guided selection.
...
FastSAM is practical for a variety of [computer vision](https://www.sfdt_ibrahim.com/glossary/computer-vision-cv) tasks that require real-time segmentation performance. Applications include:
